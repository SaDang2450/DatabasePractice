...
JOIN `new_review`
ON store.id = new_review.storeId
WHERE store.name = '배달삼겹' AND new_review.point < 3.0 ORDER BY `point`;

SELECT store.name, new_review.menu, new_review.point, new_review.review
FROM `store`
JOIN `new_review`
ON store.id = new_review.storeId
WHERE new_review.userName = '이병헌' AND new_review.point >= 4.5 AND new_review.review IS NOT NULL;

SELECT store.name, store.phoneNumber, avg(new_review.point) AS `averagePoint`
FROM `store`
JOIN `new_review`
ON store.id = new_review.storeId
GROUP BY store.id
