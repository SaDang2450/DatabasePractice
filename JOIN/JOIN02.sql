...
SELECT realtor.office, realtor.phoneNumber, real_estate.address, real_estate.area, real_estate.price
FROM `realtor`
JOIN `real_estate`
ON realtor.id = real_estate.realtorId
WHERE real_estate.type = '매매' AND real_estate.price <= 130000
ORDER BY real_estate.area DESC;

SELECT realtor.office, realtor.phoneNumber, real_estate.address, real_estate.price, real_estate.rentPrice, real_estate.area
FROM `realtor`
JOIN `real_estate`
ON realtor.id = real_estate.realtorId
WHERE realtor.grade = '안심중개사' AND real_estate.type = '월세' 
ORDER BY real_estate.rentPrice DESC;

SELECT realtor.office, realtor.phoneNumber, real_estate.address, real_estate.price, real_estate.area
FROM `realtor`
JOIN `real_estate`
ON realtor.id = real_estate.realtorId
WHERE realtor.address = '서울시 서초구' AND real_estate.type = '전세' AND real_estate.price <= 70000
ORDER BY real_estate.price;

SELECT realtor.office, count(*) AS `count`
FROM `realtor`
JOIN `real_estate`
ON realtor.id = real_estate.realtorId
GROUP BY realtor.office
ORDER BY `count` DESC;
...
JOIN `real_estate`
ON realtor.id = real_estate.realtorId
WHERE realtor.office = '대박공인중개사' AND real_estate.type IN ('매매', '전세') AND real_estate.area >= 100
ORDER BY real_estate.price;
